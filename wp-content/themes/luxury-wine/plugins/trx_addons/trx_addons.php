...
if (!function_exists('luxury_wine_trx_addons_theme_setup1')) {
	add_action( 'after_setup_theme', 'luxury_wine_trx_addons_theme_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'luxury_wine_trx_addons_theme_setup1', 8 );
	function luxury_wine_trx_addons_theme_setup1() {
		if (luxury_wine_exists_trx_addons()) {
			add_filter( 'luxury_wine_filter_list_posts_types',	'luxury_wine_trx_addons_list_post_types');
			add_filter( 'luxury_wine_filter_list_header_styles','luxury_wine_trx_addons_list_header_styles');
			add_filter( 'luxury_wine_filter_list_footer_styles','luxury_wine_trx_addons_list_footer_styles');
		}
	}
}
...
if (!function_exists('luxury_wine_trx_addons_theme_setup9')) {
	add_action( 'after_setup_theme', 'luxury_wine_trx_addons_theme_setup9', 9 );
	function luxury_wine_trx_addons_theme_setup9() {
		if (luxury_wine_exists_trx_addons()) {
			add_filter( 'trx_addons_sc_atts',							'luxury_wine_trx_addons_sc_atts', 10, 2);
			add_filter( 'trx_addons_sc_output',							'luxury_wine_trx_addons_sc_output', 10, 4);
			add_filter( 'trx_addons_filter_featured_image',				'luxury_wine_trx_addons_featured_image', 10, 2);
			add_filter( 'trx_addons_filter_no_image',					'luxury_wine_trx_addons_no_image' );
			add_filter( 'trx_addons_filter_get_list_icons',				'luxury_wine_trx_addons_get_list_icons', 10, 2 );
			add_filter( 'trx_addons_filter_slider_title',				'luxury_wine_trx_addons_slider_title', 10, 2 );
			add_filter( 'trx_addons_filter_meta_box_fields',			'luxury_wine_trx_addons_override_fields', 10, 2);
			add_action( 'wp_enqueue_scripts', 							'luxury_wine_trx_addons_frontend_scripts', 1100 );
			add_filter( 'luxury_wine_filter_query_sort_order',	 			'luxury_wine_trx_addons_query_sort_order', 10, 3);
			add_filter( 'luxury_wine_filter_merge_scripts',					'luxury_wine_trx_addons_merge_scripts');
			add_filter( 'luxury_wine_filter_get_css',						'luxury_wine_trx_addons_get_css', 10, 4);
			add_filter( 'luxury_wine_filter_prepare_css',					'luxury_wine_trx_addons_prepare_css', 10, 2);
			add_filter( 'luxury_wine_filter_prepare_js',					'luxury_wine_trx_addons_prepare_js', 10, 2);
			add_filter( 'luxury_wine_filter_get_post_categories',		 	'luxury_wine_trx_addons_get_post_categories');
			add_filter( 'luxury_wine_filter_get_post_date',		 			'luxury_wine_trx_addons_get_post_date');
			add_filter( 'trx_addons_filter_get_post_date',		 		'luxury_wine_trx_addons_get_post_date_wrap');
			add_filter( 'luxury_wine_filter_post_type_taxonomy',			'luxury_wine_trx_addons_post_type_taxonomy', 10, 2 );
			if (is_admin()) {
				add_filter( 'luxury_wine_filter_allow_override', 			'luxury_wine_trx_addons_allow_override', 10, 2);
				add_filter( 'luxury_wine_filter_allow_theme_icons', 		'luxury_wine_trx_addons_allow_theme_icons', 10, 2);
				add_filter( 'trx_addons_sc_map',						'luxury_wine_trx_addons_sc_map', 10, 2);
			} else {
				add_filter( 'trx_addons_filter_theme_logo',				'luxury_wine_trx_addons_theme_logo');
				add_filter( 'trx_addons_filter_post_meta',				'luxury_wine_trx_addons_post_meta', 10, 2);
				add_filter( 'luxury_wine_filter_get_mobile_menu',			'luxury_wine_trx_addons_get_mobile_menu');
				add_filter( 'luxury_wine_filter_detect_blog_mode',			'luxury_wine_trx_addons_detect_blog_mode' );
				add_filter( 'luxury_wine_filter_get_blog_title', 			'luxury_wine_trx_addons_get_blog_title');
				add_action( 'luxury_wine_action_login',						'luxury_wine_trx_addons_action_login', 10, 2);
				add_action( 'luxury_wine_action_search',					'luxury_wine_trx_addons_action_search', 10, 3);
				add_action( 'luxury_wine_action_breadcrumbs',				'luxury_wine_trx_addons_action_breadcrumbs');
				add_action( 'luxury_wine_action_show_layout',				'luxury_wine_trx_addons_action_show_layout', 10, 1);
			}
		}
		
		// Add this filter any time: if plugin exists - load plugin's styles, if not exists - load layouts.css instead plugin's styles
		add_filter( 'luxury_wine_filter_merge_styles',						'luxury_wine_trx_addons_merge_styles');
		
		if (is_admin()) {
			add_filter( 'luxury_wine_filter_tgmpa_required_plugins',		'luxury_wine_trx_addons_tgmpa_required_plugins' );
			add_action( 'admin_enqueue_scripts', 						'luxury_wine_trx_addons_editor_load_scripts_admin');
		}
	}
}

// Filter to add in the required plugins list
if ( !function_exists( 'luxury_wine_trx_addons_tgmpa_required_plugins' ) ) {
	//Handler of the add_filter('luxury_wine_filter_tgmpa_required_plugins',	'luxury_wine_trx_addons_tgmpa_required_plugins');
	function luxury_wine_trx_addons_tgmpa_required_plugins($list=array()) {
		if (in_array('trx_addons', luxury_wine_storage_get('required_plugins'))) {
			$path = luxury_wine_get_file_dir('plugins/trx_addons/trx_addons.zip');
			$list[] = array(
					'name' 		=> esc_html__('ThemeREX Addons', 'luxury-wine'),
					'slug' 		=> 'trx_addons',
					'version'	=> '1.6.16.02',
...
if ( !function_exists( 'luxury_wine_exists_trx_addons' ) ) {
	function luxury_wine_exists_trx_addons() {
		return defined('TRX_ADDONS_VERSION');
	}
}

// Return true if team is supported
if ( !function_exists( 'luxury_wine_exists_team' ) ) {
	function luxury_wine_exists_team() {
		return defined('TRX_ADDONS_CPT_TEAM_PT');
	}
}

// Return true if services is supported
if ( !function_exists( 'luxury_wine_exists_services' ) ) {
	function luxury_wine_exists_services() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT');
	}
}

// Return true if portfolio is supported
if ( !function_exists( 'luxury_wine_exists_portfolio' ) ) {
	function luxury_wine_exists_portfolio() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT');
	}
}

// Return true if courses is supported
if ( !function_exists( 'luxury_wine_exists_courses' ) ) {
	function luxury_wine_exists_courses() {
		return defined('TRX_ADDONS_CPT_COURSES_PT');
	}
}

// Return true if layouts is supported
if ( !function_exists( 'luxury_wine_exists_layouts' ) ) {
	function luxury_wine_exists_layouts() {
		return defined('TRX_ADDONS_CPT_LAYOUTS_PT');
	}
}

// Return true if dishes is supported
if ( !function_exists( 'luxury_wine_exists_dishes' ) ) {
	function luxury_wine_exists_dishes() {
		return defined('TRX_ADDONS_CPT_DISHES_PT');
	}
}

// Return true if it's team page
if ( !function_exists( 'luxury_wine_is_team_page' ) ) {
	function luxury_wine_is_team_page() {
		return defined('TRX_ADDONS_CPT_TEAM_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'luxury_wine_is_services_page' ) ) {
	function luxury_wine_is_services_page() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'luxury_wine_is_portfolio_page' ) ) {
	function luxury_wine_is_portfolio_page() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'luxury_wine_is_courses_page' ) ) {
	function luxury_wine_is_courses_page() {
		return defined('TRX_ADDONS_CPT_COURSES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'luxury_wine_is_dishes_page' ) ) {
	function luxury_wine_is_dishes_page() {
		return defined('TRX_ADDONS_CPT_DISHES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'luxury_wine_trx_addons_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'luxury_wine_trx_addons_frontend_scripts', 1100 );
	function luxury_wine_trx_addons_frontend_scripts() {
		if (luxury_wine_exists_trx_addons()) {
			if (luxury_wine_is_on(luxury_wine_get_theme_option('debug_mode')) && luxury_wine_get_file_dir('plugins/trx_addons/trx_addons.css')!='') {
				wp_enqueue_style( 'luxury-wine-trx_addons',  luxury_wine_get_file_url('plugins/trx_addons/trx_addons.css'), array(), null );
				wp_enqueue_style( 'luxury-wine-trx_addons_editor',  luxury_wine_get_file_url('plugins/trx_addons/trx_addons.editor.css'), array(), null );
			}
			if (luxury_wine_is_on(luxury_wine_get_theme_option('debug_mode')) && luxury_wine_get_file_dir('plugins/trx_addons/trx_addons.js')!='')
				wp_enqueue_script( 'luxury-wine-trx_addons', luxury_wine_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		} else {
			// Load custom layouts from the theme if plugin not exists
			if ( luxury_wine_is_on(luxury_wine_get_theme_option('debug_mode')) ) {
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/layouts.css') );
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/logo.css') );
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/menu.css') );
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/search.css') );
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/title.css') );
				wp_enqueue_style( 'luxury-wine-layouts', luxury_wine_get_file_url('plugins/trx_addons/layouts/featured.css') );
			}
		}
	}
}
...
// Merge custom styles
if ( !function_exists( 'luxury_wine_trx_addons_merge_styles' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_merge_styles', 'luxury_wine_trx_addons_merge_styles');
	function luxury_wine_trx_addons_merge_styles($list) {
...
if ( !function_exists( 'luxury_wine_trx_addons_merge_scripts' ) ) {
	//Handler of the add_filter('luxury_wine_filter_merge_scripts', 'luxury_wine_trx_addons_merge_scripts');
	function luxury_wine_trx_addons_merge_scripts($list) {
...
if ( !function_exists( 'luxury_wine_trx_addons_detect_blog_mode' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_detect_blog_mode', 'luxury_wine_trx_addons_detect_blog_mode' );
	function luxury_wine_trx_addons_detect_blog_mode($mode='') {
		if ( luxury_wine_is_team_page() )
			$mode = 'team';
		else if ( luxury_wine_is_services_page() )
			$mode = 'services';
		else if ( luxury_wine_is_portfolio_page() )
			$mode = 'portfolio';
		else if ( luxury_wine_is_courses_page() )
			$mode = 'courses';
		else if ( luxury_wine_is_dishes_page() )
			$mode = 'dishes';
		return $mode;
	}
}
...
if ( !function_exists( 'luxury_wine_trx_addons_list_post_types' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_list_posts_types', 'luxury_wine_trx_addons_list_post_types');
	function luxury_wine_trx_addons_list_post_types($list=array()) {
...
if ( !function_exists( 'luxury_wine_trx_addons_post_type_taxonomy' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_post_type_taxonomy',	'luxury_wine_trx_addons_post_type_taxonomy', 10, 2 );
	function luxury_wine_trx_addons_post_type_taxonomy($tax='', $post_type='') {
...
if ( !function_exists( 'luxury_wine_trx_addons_get_post_categories' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_get_post_categories', 		'luxury_wine_trx_addons_get_post_categories');
	function luxury_wine_trx_addons_get_post_categories($cats='') {

		if ( defined('TRX_ADDONS_CPT_TEAM_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_TEAM_PT) {
				$cats = luxury_wine_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_TEAM_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_SERVICES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_SERVICES_PT) {
				$cats = luxury_wine_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_PORTFOLIO_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_PORTFOLIO_PT) {
				$cats = luxury_wine_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_PORTFOLIO_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				$cats = luxury_wine_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_COURSES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_DISHES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_DISHES_PT) {
				$cats = luxury_wine_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_DISHES_TAXONOMY);
			}
		}
		return $cats;
	}
...
if ( !function_exists( 'luxury_wine_trx_addons_list_header_styles' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_list_header_styles', 'luxury_wine_trx_addons_list_header_styles');
	function luxury_wine_trx_addons_list_header_styles($list=array()) {
		if (luxury_wine_exists_layouts()) {
			$layouts = luxury_wine_get_list_posts(false, array(
...
if ( !function_exists( 'luxury_wine_trx_addons_list_footer_styles' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_list_footer_styles', 'luxury_wine_trx_addons_list_footer_styles');
	function luxury_wine_trx_addons_list_footer_styles($list=array()) {
		if (luxury_wine_exists_layouts()) {
			$layouts = luxury_wine_get_list_posts(false, array(
							'post_type' => TRX_ADDONS_CPT_LAYOUTS_PT,
							'meta_key' => 'trx_addons_layout_type',
...
if ( !function_exists( 'luxury_wine_trx_addons_get_post_date_wrap' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_post_date', 'luxury_wine_trx_addons_get_post_date_wrap');
	function luxury_wine_trx_addons_get_post_date_wrap($dt='') {
		return apply_filters('luxury_wine_filter_get_post_date', $dt);
	}
}

// Show date of the courses
if ( !function_exists( 'luxury_wine_trx_addons_get_post_date' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_get_post_date', 'luxury_wine_trx_addons_get_post_date');
	function luxury_wine_trx_addons_get_post_date($dt='') {
		...
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				...
				$dt = sprintf($dt < date('Y-m-d') 
						? esc_html__('Started on %s', 'luxury-wine') 
						: esc_html__('Starting %s', 'luxury-wine'), 
...
if (!function_exists('luxury_wine_trx_addons_allow_override')) {
	//Handler of the add_filter( 'luxury_wine_filter_allow_override', 'luxury_wine_trx_addons_allow_override', 10, 2);
	function luxury_wine_trx_addons_allow_override($allow, $post_type) {
		return $allow
...
if (!function_exists('luxury_wine_trx_addons_allow_theme_icons')) {
	//Handler of the add_filter( 'luxury_wine_filter_allow_theme_icons', 'luxury_wine_trx_addons_allow_theme_icons', 10, 2);
	function luxury_wine_trx_addons_allow_theme_icons($allow, $post_type) {
		return $allow
					|| (defined('TRX_ADDONS_CPT_LAYOUTS_PT') && $post_type==TRX_ADDONS_CPT_LAYOUTS_PT);
	}
}

// Add fields into override
if (!function_exists('luxury_wine_trx_addons_override_fields')) {
	//Handler of the add_filter( 'trx_addons_filter_override_fields', 'luxury_wine_trx_addons_override_fields', 10, 2);
	function luxury_wine_trx_addons_override_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'luxury-wine'),
				"desc" => wp_kses_data( __("Team member's email", 'luxury-wine') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
if ( !function_exists( 'luxury_wine_trx_addons_editor_load_scripts_admin' ) ) {
	//Handler of the add_action("admin_enqueue_scripts", 'luxury_wine_trx_addons_editor_load_scripts_admin');
	function luxury_wine_trx_addons_editor_load_scripts_admin() {
		// Add styles in the WP text editor
		add_editor_style( array(
							luxury_wine_get_file_url('plugins/trx_addons/trx_addons.editor.css')
							)
						 );	
	}
}
...
if ( !function_exists( 'luxury_wine_trx_addons_sc_atts' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_atts', 'luxury_wine_trx_addons_sc_atts', 10, 2);
	function luxury_wine_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'luxury_wine_trx_addons_sc_map' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_map', 'luxury_wine_trx_addons_sc_map', 10, 2);
	function luxury_wine_trx_addons_sc_map($params, $sc) {
...
if ( !function_exists( 'luxury_wine_trx_addons_sc_output' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_output', 'luxury_wine_trx_addons_sc_output', 10, 4);
	function luxury_wine_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_layouts_featured')))
			luxury_wine_sc_layouts_showed('featured', is_singular() && in_array(get_post_type(), array('post', 'page')) && has_post_thumbnail());
			...
			luxury_wine_sc_layouts_showed('cart', true);
		else if (in_array($sc, array('trx_sc_layouts_logo')))
			luxury_wine_sc_layouts_showed('logo', true);
		else if (in_array($sc, array('trx_sc_layouts_menu')))
			luxury_wine_sc_layouts_showed('menu', true);
		else if (in_array($sc, array('trx_sc_layouts_search')))
			luxury_wine_sc_layouts_showed('search', true);
...
if ( !function_exists( 'luxury_wine_trx_addons_slider_title' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_slider_title',	'luxury_wine_trx_addons_slider_title', 10, 2 );
	function luxury_wine_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if (!function_exists('luxury_wine_sc_layouts_showed')) {
	function luxury_wine_sc_layouts_showed($name, $val=null) {
		if ($val!==null)
			return luxury_wine_storage_set_array('sc_layouts_components', $name, $val);
		else
			return luxury_wine_storage_get_array('sc_layouts_components', $name) != '';
	}
}

// Return image size multiplier
if (!function_exists('luxury_wine_get_retina_multiplier')) {
	function luxury_wine_get_retina_multiplier($force_retina=0) {
		static $mult = 0;
...
if (!function_exists('luxury_wine_build_slider_layout')) {
	function luxury_wine_build_slider_layout($args) {
...
if ( !function_exists( 'luxury_wine_trx_addons_featured_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_featured_image', 'luxury_wine_trx_addons_featured_image', 10, 2);
	function luxury_wine_trx_addons_featured_image($processed=false, $args=array()) {
		$args['show_no_image'] = true;
		$args['singular'] = false;
		$args['hover'] = luxury_wine_get_theme_option('image_hover');
		luxury_wine_show_post_featured($args);
		return true;
	}
}

// Return theme specific 'no-image' picture
if ( !function_exists( 'luxury_wine_trx_addons_no_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_no_image', 'luxury_wine_trx_addons_no_image');
	function luxury_wine_trx_addons_no_image($no_image='') {
		return luxury_wine_get_no_image($no_image);
	}
}

// Return theme-specific icons
if ( !function_exists( 'luxury_wine_trx_addons_get_list_icons' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_list_icons', 'luxury_wine_trx_addons_get_list_icons', 10, 2 );
	function luxury_wine_trx_addons_get_list_icons($list, $prepend_inherit) {
		return luxury_wine_get_list_icons($prepend_inherit);
	}
}

// Return links to the social profiles
if (!function_exists('luxury_wine_get_socials_links')) {
	function luxury_wine_get_socials_links($style='icons') {
...
if (!function_exists('luxury_wine_get_share_links')) {
	function luxury_wine_get_share_links($args=array()) {
...
if (!function_exists('luxury_wine_show_share_links')) {
	function luxury_wine_show_share_links($args=array()) {
...
if (!function_exists('luxury_wine_get_category_image')) {
	function luxury_wine_get_category_image($term_id=0) {
...
if (!function_exists('luxury_wine_get_category_icon')) {
	function luxury_wine_get_category_icon($term_id=0) {
...
if (!function_exists('luxury_wine_get_post_counters')) {
	function luxury_wine_get_post_counters($counters='views') {
...
if (!function_exists('luxury_wine_get_list_animations_in')) {
	function luxury_wine_get_list_animations_in() {
...
if (!function_exists('luxury_wine_get_animation_classes')) {
	function luxury_wine_get_animation_classes($animation, $speed='normal', $loop='none') {
...
if (!function_exists('luxury_wine_get_comment_counters')) {
	function luxury_wine_get_comment_counters($counters = 'likes') {
...
if (!function_exists('luxury_wine_show_comment_counters')) {
	function luxury_wine_show_comment_counters($counters = 'likes') {
...
if (!function_exists('luxury_wine_trx_addons_query_sort_order')) {
	//Handler of the add_filter('luxury_wine_filter_query_sort_order', 'luxury_wine_trx_addons_query_sort_order', 10, 3);
	function luxury_wine_trx_addons_query_sort_order($q=array(), $orderby='date', $order='desc') {
		if ($orderby == 'views') {
...
if ( !function_exists( 'luxury_wine_trx_addons_theme_logo' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_theme_logo', 'luxury_wine_trx_addons_theme_logo');
	function luxury_wine_trx_addons_theme_logo($logo) {
		return luxury_wine_get_logo_image();
	}
}

// Return theme-specific post meta to the plugin
if ( !function_exists( 'luxury_wine_trx_addons_post_meta' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_post_meta',	'luxury_wine_trx_addons_post_meta', 10, 2);
	function luxury_wine_trx_addons_post_meta($meta, $args=array()) {
		return luxury_wine_show_post_meta($args);
	}
}
	
// Redirect action 'get_mobile_menu' to the plugin
// Return stored items as mobile menu
if ( !function_exists( 'luxury_wine_trx_addons_get_mobile_menu' ) ) {
	//Handler of the add_filter("luxury_wine_filter_get_mobile_menu", 'luxury_wine_trx_addons_get_mobile_menu');
	function luxury_wine_trx_addons_get_mobile_menu($menu) {
		return apply_filters('trx_addons_filter_get_mobile_menu', $menu);
	}
}

// Redirect action 'login' to the plugin
if (!function_exists('luxury_wine_trx_addons_action_login')) {
	//Handler of the add_action( 'luxury_wine_action_login',		'luxury_wine_trx_addons_action_login', 10, 2);
	function luxury_wine_trx_addons_action_login($link_text='', $link_title='') {
		do_action( 'trx_addons_action_login', $link_text, $link_title );
	}
}

// Redirect action 'search' to the plugin
if (!function_exists('luxury_wine_trx_addons_action_search')) {
	//Handler of the add_action( 'luxury_wine_action_search', 'luxury_wine_trx_addons_action_search', 10, 3);
	function luxury_wine_trx_addons_action_search($style, $class, $ajax) {
		do_action( 'trx_addons_action_search', $style, $class, $ajax );
	}
}

// Redirect action 'breadcrumbs' to the plugin
if (!function_exists('luxury_wine_trx_addons_action_breadcrumbs')) {
	//Handler of the add_action( 'luxury_wine_action_breadcrumbs',	'luxury_wine_trx_addons_action_breadcrumbs');
	function luxury_wine_trx_addons_action_breadcrumbs() {
		do_action( 'trx_addons_action_breadcrumbs' );
	}
}

// Redirect action 'show_layout' to the plugin
if (!function_exists('luxury_wine_trx_addons_action_show_layout')) {
	//Handler of the add_action( 'luxury_wine_action_show_layout', 'luxury_wine_trx_addons_action_show_layout', 10, 1);
	function luxury_wine_trx_addons_action_show_layout($layout_id='') {
		do_action( 'trx_addons_action_show_layout', $layout_id );
	}
}

// Redirect filter 'get_blog_title' to the plugin
if ( !function_exists( 'luxury_wine_trx_addons_get_blog_title' ) ) {
	//Handler of the add_filter( 'luxury_wine_filter_get_blog_title', 'luxury_wine_trx_addons_get_blog_title');
	function luxury_wine_trx_addons_get_blog_title($title='') {
		return apply_filters('trx_addons_filter_get_blog_title', $title);
	}
}

// Redirect filter 'prepare_css' to the plugin
if (!function_exists('luxury_wine_trx_addons_prepare_css')) {
	//Handler of the add_filter( 'luxury_wine_filter_prepare_css',	'luxury_wine_trx_addons_prepare_css', 10, 2);
	function luxury_wine_trx_addons_prepare_css($css='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_css', $css, $remove_spaces );
	}
}

// Redirect filter 'prepare_js' to the plugin
if (!function_exists('luxury_wine_trx_addons_prepare_js')) {
	//Handler of the add_filter( 'luxury_wine_filter_prepare_js',	'luxury_wine_trx_addons_prepare_js', 10, 2);
	function luxury_wine_trx_addons_prepare_js($js='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_js', $js, $remove_spaces );
	}
}

// Return text for the "I agree ..." checkbox
if ( ! function_exists( 'luxury_wine_trx_addons_privacy_text' ) ) {
    add_filter( 'trx_addons_filter_privacy_text', 'luxury_wine_trx_addons_privacy_text' );
    function luxury_wine_trx_addons_privacy_text( $text='' ) {
        return luxury_wine_get_privacy_text();
    }
}

// Add theme-specific options to the post's options
if (!function_exists('luxury_wine_trx_addons_override_options')) {
    add_filter( 'trx_addons_filter_override_options', 'luxury_wine_trx_addons_override_options');
    function luxury_wine_trx_addons_override_options($options=array()) {
        return apply_filters('luxury_wine_filter_override_options', $options);
    }
}
...
if (!function_exists('luxury_wine_trx_addons_get_css')) {
	//Handler of the add_filter('luxury_wine_filter_get_css', 'luxury_wine_trx_addons_get_css', 10, 4);
	function luxury_wine_trx_addons_get_css($css, $colors, $fonts, $scheme='') {
...
.widget_contacts .contacts_info span:before {
	...
	border-color: {$colors['alter_hover_05']};
}
...
.mc4wp-form .mc4wp-form-fields .mc-btn:hover,
...
.sc_item_button a.sc_button_simple:not(.sc_button_bg_image):hover:after {
	color: {$colors['inverse_text']};
	border-color: {$colors['simple_bg_hover']} !important;
	background-color: {$colors['simple_bg_hover']} !important;
}
...
.sc_blogger.slider_swiper .swiper-pagination-bullet {
	border-color: {$colors['accent2_hover_05']};
}
...
.sc_blogger_creative .sc_blogger_item {
	background-color: {$colors['bg_color']};
}
.sc_blogger_classic .sc_blogger_item_header .post_date .day,
.sc_blogger_creative .sc_blogger_item_header .post_date .day {
	color: {$colors['accent2_hover_05']};
}

.sc_blogger_default .post_meta .post_meta_item,
.sc_blogger_default .post_meta .post_meta_item:before,
.sc_blogger_default .post_meta a,
...
.sc_services_default .post_featured:hover + .sc_services_item_info .sc_services_item_title a{
	color: {$colors['text_link']};	
}
...
.sc_skills .sc_skills_item_subtitle {
	color:{$colors['text_link']};
}
...
.sc_testimonials_item_content:before {
	color: {$colors['accent2_hover_005']};
}
.sc_testimonials_item_author_title {
	color: {$colors['text_dark']};
}
.sc_testimonials_item_author_subtitle {
	color: {$colors['accent2_hover']};
}
...
.sc_team_default .post_featured:hover + .sc_team_item_info .sc_team_item_title a{
	color: {$colors['text_link']};	
}
