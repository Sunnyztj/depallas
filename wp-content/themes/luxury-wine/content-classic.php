...
$luxury_wine_columns = $luxury_wine_blog_style[0] == 'masonry' ? $luxury_wine_columns : 1;
...
?><div class="<?php echo trim($luxury_wine_blog_style[0]) == 'classic' ? 'column' : 'masonry_item masonry_item'; ?>-1_<?php echo esc_attr($luxury_wine_columns); ?>"><article id="post-<?php the_ID(); ?>"
	<?php post_class( 'post_item post_format_'.esc_attr($luxury_wine_post_format)
					. ' post_layout_classic post_layout_classic_'.esc_attr($luxury_wine_columns)
					. ' post_layout_'.esc_attr($luxury_wine_blog_style[0]) 
					. ' post_layout_'.esc_attr($luxury_wine_blog_style[0]).'_'.esc_attr($luxury_wine_columns)
					); ?>
	...
	// Featured image
	luxury_wine_show_post_featured( array( 'thumb_size' => luxury_wine_get_thumb_size($luxury_wine_blog_style[0] == 'masonry' ? 
									   (strpos(luxury_wine_get_theme_option('body_style'), 'full')!==false 
											? ( $luxury_wine_columns > 2 ? 'masonry-big' : 'full' )
											: (	$luxury_wine_columns <= 2 && $luxury_wine_expanded ? 'masonry-big' : 'masonry')
										)
										: 'classic') ) );
	?>
	...
	<!--
		<div class="post_content_inner">
			<?php
			$luxury_wine_show_learn_more = true;
			if (has_excerpt()) {
				the_excerpt();
