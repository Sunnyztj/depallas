/* global luxury_wine_color_schemes, luxury_wine_dependencies, Color */
...
( function( api ) {
	...
	for (var i in luxury_wine_color_schemes) {
		cssTemplate[i] = wp.template( 'luxury-wine-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'luxury-wine-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+luxury_wine_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(luxury_wine_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', luxury_wine_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', luxury_wine_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			luxury_wine_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		luxury_wine_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		luxury_wine_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		luxury_wine_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function luxury_wine_customizer_check_dependencies(cont) {
		"use strict";
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in luxury_wine_dependencies) {
				if (fld == id) {
					depend = luxury_wine_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('luxury_wine_options_no_use');
				} else {
					jQuery(this).hide().addClass('luxury_wine_options_no_use');
				}
			}
		});
	}
	...
	function luxury_wine_customizer_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme_editor') {
			luxury_wine_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in luxury_wine_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					luxury_wine_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							luxury_wine_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							luxury_wine_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in luxury_wine_theme_fonts) {
					for (rule in luxury_wine_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							luxury_wine_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) luxury_wine_customizer_update_css();
		}
	...
	// Actions buttons
	function luxury_wine_customizer_actions(e) {
		"use strict";
	...
	// Store new value in the color table
	function luxury_wine_customizer_update_color_scheme(opt, value) {
		"use strict";
		luxury_wine_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(luxury_wine_serialize(luxury_wine_color_schemes))
	}
	
	// Store new value in the theme fonts
	function luxury_wine_customizer_update_theme_fonts(opt, rule, value) {
		"use strict";
		luxury_wine_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function luxury_wine_customizer_update_load_fonts() {
		"use strict";
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=luxury_wine_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in luxury_wine_theme_fonts) {
	...
	function luxury_wine_customizer_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in luxury_wine_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( luxury_wine_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', luxury_wine_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', luxury_wine_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		luxury_wine_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function luxury_wine_customizer_update_css() {
		"use strict";
		...
		var fonts = luxury_wine_customizer_add_theme_fonts(luxury_wine_theme_fonts);

		// Make styles and add into css
		css += luxury_wine_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in luxury_wine_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in luxury_wine_color_schemes[scheme].colors) {
				colors[i] = luxury_wine_color_schemes[scheme].colors[i];
			}
	...
	function luxury_wine_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + luxury_wine_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + luxury_wine_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + luxury_wine_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function luxury_wine_customizer_prepare_css_value(val) {
		"use strict";
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
	...
	function luxury_wine_customizer_prepare_html_value(val) {
		"use strict";
