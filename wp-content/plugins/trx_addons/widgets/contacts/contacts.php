...
class trx_addons_widget_contacts extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_contacts', 'description' => esc_html__('Contacts - logo and short description, address, phone, skype and email', 'trx_addons'));
	...
	function widget($args, $instance) {
		...
		set_query_var('trx_addons_args_widget_contacts', array_merge($args, compact('title', 'logo', 'logo_retina', 'description', 'content', 'email', 'address', 'phone', 'skype', 'socials', 'googlemap', 'googlemap_height', 'googlemap_position')));

		if (($fdir = trx_addons_get_file_dir('widgets/contacts/tpl.default.php')) != '') { include $fdir; }
	}
	...
	function update($new_instance, $old_instance) {
		...
		$instance['skype'] = wp_kses_data($new_instance['skype']);
	...
	function form($instance) {
		...
		<p>
			...
			<textarea id="<?php echo esc_attr($this->get_field_id('phone')); ?>" name="<?php echo esc_attr($this->get_field_name('phone')); ?>" class="widgets_param_fullwidth"><?php echo esc_html($phone); ?></textarea>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('skype')); ?>"><?php esc_html_e('Skype:', 'trx_addons'); ?></label><br>
			<input id="<?php echo esc_attr($this->get_field_id('skype')); ?>" name="<?php echo esc_attr($this->get_field_name('skype')); ?>" value="<?php echo esc_attr($skype); ?>" class="widgets_param_fullwidth">
		</p>
		...
		<p>
			...
			<input type="radio" id="<?php echo esc_attr($this->get_field_id('style')); ?>_left" name="<?php echo esc_attr($this->get_field_name('googlemap_position')); ?>" value="left" <?php echo ('left'==$googlemap_position ? ' checked="checked"' : ''); ?> />
			<label for="<?php echo esc_attr($this->get_field_id('googlemap_position')); ?>_left"><?php esc_html_e('Left', 'trx_addons'); ?></label>
			<input type="radio" id="<?php echo esc_attr($this->get_field_id('style')); ?>_right" name="<?php echo esc_attr($this->get_field_name('googlemap_position')); ?>" value="right" <?php echo ('right'==$googlemap_position ? ' checked="checked"' : ''); ?> />
...
if ( !function_exists( 'trx_addons_sc_widget_contacts' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_contacts", "trx_addons_sc_widget_contacts");
}
...
if (!function_exists('trx_addons_sc_widget_contacts_add_in_vc')) {
		...
		vc_map( apply_filters('trx_addons_sc_map', array(
				...
				"params" => array_merge(
					array(
						...
						array(
							"param_name" => "skype",
							"heading" => esc_html__("Skype", 'trx_addons'),
							"description" => wp_kses_data( __("Your skype", 'trx_addons') ),
							"admin_label" => true,
							"type" => "textfield"
						),
						array(
	...
	if (trx_addons_exists_visual_composer()) add_action('after_setup_theme', 'trx_addons_sc_widget_contacts_add_in_vc', 20);
}
?>