...
if ( !function_exists( 'trx_addons_sc_layouts_cart' ) ) {
	function trx_addons_sc_layouts_cart($atts, $content=null){	
		...
		ob_start();
		set_query_var('trx_addons_args_sc_layouts_cart', $atts);
	/*	if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/cart/tpl.'.trx_addons_esc($atts['type']).'.php')) != '') { include $fdir; }
		else */ if (($fdir = trx_addons_get_file_dir('cpt/layouts/shortcodes/cart/tpl.default.php')) != '') { include $fdir; }
		$output = ob_get_contents();
		ob_end_clean();
		
		return apply_filters('trx_addons_sc_output', $output, 'trx_sc_layouts_cart', $atts, $content);
	}
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_sc_layouts_cart", "trx_addons_sc_layouts_cart");
}
...
if (!function_exists('trx_addons_sc_layouts_cart_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('after_setup_theme', 'trx_addons_sc_layouts_cart_add_in_vc', 15);
}
?>