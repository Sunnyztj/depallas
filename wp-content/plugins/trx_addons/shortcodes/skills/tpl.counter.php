	...
	$data .= ($args['columns'] > 0 ? '<div class="sc_skills_column '.esc_attr(trx_addons_get_column_class(1, $args['columns'])).'">' : '')
			. '<div class="sc_skills_item_wrap">'
				. '<div class="sc_skills_item">'
					...
					. '</div>'
				. '</div>'
				. (!empty($v['subtitle']) ? '<div class="sc_skills_item_subtitle">'.nl2br(str_replace('|', "\n", esc_html($v['subtitle']))).'</div>' : '')
				. (!empty($v['title']) ? '<h4 class="sc_skills_item_title">'.nl2br(str_replace('|', "\n", esc_html($v['title']))).'</h4>' : '')
			. '</div>'
		. ($args['columns'] > 0 ? '</div>' : '');
}
