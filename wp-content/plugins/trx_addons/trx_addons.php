...
// Current version
if ( ! defined( 'TRX_ADDONS_VERSION' ) ) define( 'TRX_ADDONS_VERSION', '1.6.16.03' );
...
if ( !function_exists('trx_addons_init') ) {
	...
	function trx_addons_init() {
		...
		$thumb_sizes = apply_filters('trx_addons_filter_add_thumb_sizes', array(
			...
			'trx_addons-thumb-blogger-classic'		=> array(555, 420, true),	
			'trx_addons-thumb-blogger-creative'		=> array(420, 364, true),	 //array(385, 334, true),	
			)
		);
...
if ( !function_exists('trx_addons_image_sizes') ) {
	...
	function trx_addons_image_sizes( $sizes ) {
		$thumb_sizes = apply_filters('trx_addons_filter_add_thumb_sizes', array(
			...
			'trx_addons-thumb-blogger-classic'	=> esc_html__( 'Classic', 'trx_addons' ),
			)
		);
		$mult = trx_addons_get_option('retina_ready', 1);
		foreach($thumb_sizes as $k=>$v) {
